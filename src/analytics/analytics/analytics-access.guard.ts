...
import { ANALYTICS_ACCESS_KEY } from './analytics-access.decorator';

@Injectable()
export class AnalyticAccessGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // -----------------------
    // STEP 1: Global Analytic Permissions
    // -----------------------
    const hasAnyBasicModule = privileges.some(
      (p) => p.module === 'basic_modules',
    );

    const hasAnalyticsModule = privileges.some(
      (p) => p.module === 'basic_modules' && p.name === 'analytics_module',
    );

    const canViewAnalytics =
      hasAnalyticsModule &&
      privileges.some(
        (p) => p.module === 'analytics_module' && p.name === 'view_analytics',
      );
    const canEditAnalytics =
      hasAnalyticsModule &&
      privileges.some(
        (p) => p.module === 'analytics_module' && p.name === 'edit_analytics',
      );
    const canCreateAnalytics =
      hasAnalyticsModule &&
      privileges.some(
        (p) => p.module === 'analytics_module' && p.name === 'create_analytics',
      );
    const canDeleteAnalytics =
      hasAnalyticsModule &&
      privileges.some(
        (p) => p.module === 'analytics_module' && p.name === 'delete_analytics',
      );
    ...
    const determineAccess = (canAction: boolean) => {
      ...
      if (!hasAnalyticsModule) return false;
      if (!canAction && hasTenantOrBUAccess) return false;
      return true;
    };

    const checkAccessForView = determineAccess(canViewAnalytics);
    const checkAccessForEdit = determineAccess(canEditAnalytics);
    const checkAccessForDelete = determineAccess(canDeleteAnalytics);
    const checkAccessForCreate = determineAccess(canCreateAnalytics);

    // -----------------------
    // STEP 4: Attach Permissions to Request
    // -----------------------
    // User can manage Analytics if they have either:
    ...
    request.permission = {
      ...
      allowAllBU,
      tenantName,
      canViewAnalytics: canViewAnalytics ? true : checkAccessForView,
      canEditAnalytics: canEditAnalytics ? true : checkAccessForEdit,
      canCreateAnalytics: canCreateAnalytics ? true : checkAccessForCreate,
      canDeleteAnalytics: canDeleteAnalytics ? true : checkAccessForDelete,
    };
