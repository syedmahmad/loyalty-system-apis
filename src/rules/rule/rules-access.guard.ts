...
import { RULES_ACCESS_KEY } from './rules-access.decorator';
...
export class RuleAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // -----------------------
    // STEP 1: Global Rule Permissions
    // -----------------------
    const hasAnyBasicModule = privileges.some(
      (p) => p.module === 'basic_modules',
    );

    const hasRulesModule = privileges.some(
      (p) => p.module === 'basic_modules' && p.name === 'rules_module',
    );

    const canViewRules =
      hasRulesModule &&
      privileges.some(
        (p) => p.module === 'rules_module' && p.name === 'view_rule',
      );
    const canEditRules =
      hasRulesModule &&
      privileges.some(
        (p) => p.module === 'rules_module' && p.name === 'edit_rule',
      );
    const canCreateRules =
      hasRulesModule &&
      privileges.some(
        (p) => p.module === 'rules_module' && p.name === 'create_rule',
      );
    const canDeleteRules =
      hasRulesModule &&
      privileges.some(
        (p) => p.module === 'rules_module' && p.name === 'delete_rule',
      );
    ...
    const determineAccess = (canAction: boolean) => {
      ...
      if (!hasRulesModule) return false;
      if (!canAction && hasTenantOrBUAccess) return false;
      return true;
    };

    const checkAccessForView = determineAccess(canViewRules);
    const checkAccessForEdit = determineAccess(canEditRules);
    const checkAccessForDelete = determineAccess(canDeleteRules);
    const checkAccessForCreate = determineAccess(canCreateRules);

    // -----------------------
    // STEP 4: Attach Permissions to Request
    // -----------------------
    // User can manage Rules if they have either:
    // 1) Global Rule permissions, or
    ...
    request.permission = {
      ...
      allowAllBU,
      tenantName,
      canViewRules: canViewRules ? true : checkAccessForView,
      canEditRules: canEditRules ? true : checkAccessForEdit,
      canCreateRules: canCreateRules ? true : checkAccessForCreate,
      canDeleteRules: canDeleteRules ? true : checkAccessForDelete,
    };
