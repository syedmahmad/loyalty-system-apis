...
import { TIERS_ACCESS_KEY } from './tiers-access.decorator';
...
export class TierAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // -----------------------
    // STEP 1: Global Tier Permissions
    // -----------------------
    const hasAnyBasicModule = privileges.some(
      (p) => p.module === 'basic_modules',
    );

    const hasTiersModule = privileges.some(
      (p) => p.module === 'basic_modules' && p.name === 'tiers_module',
    );

    const canViewTiers =
      hasTiersModule &&
      privileges.some(
        (p) => p.module === 'tiers_module' && p.name === 'view_tier',
      );
    const canEditTiers =
      hasTiersModule &&
      privileges.some(
        (p) => p.module === 'tiers_module' && p.name === 'edit_tier',
      );
    const canCreateTiers =
      hasTiersModule &&
      privileges.some(
        (p) => p.module === 'tiers_module' && p.name === 'create_tier',
      );
    const canDeleteTiers =
      hasTiersModule &&
      privileges.some(
        (p) => p.module === 'tiers_module' && p.name === 'delete_tier',
      );
    ...
    const determineAccess = (canAction: boolean) => {
      ...
      if (!hasTiersModule) return false;
      if (!canAction && hasTenantOrBUAccess) return false;
      return true;
    };

    const checkAccessForView = determineAccess(canViewTiers);
    const checkAccessForEdit = determineAccess(canEditTiers);
    const checkAccessForDelete = determineAccess(canDeleteTiers);
    const checkAccessForCreate = determineAccess(canCreateTiers);

    // -----------------------
    // STEP 4: Attach Permissions to Request
    // -----------------------
    // User can manage Tiers if they have either:
    // 1) Global Tier permissions, or
    ...
    request.permission = {
      ...
      allowAllBU,
      tenantName,
      canViewTiers: canViewTiers ? true : checkAccessForView,
      canEditTiers: canEditTiers ? true : checkAccessForEdit,
      canCreateTiers: canCreateTiers ? true : checkAccessForCreate,
      canDeleteTiers: canDeleteTiers ? true : checkAccessForDelete,
    };
