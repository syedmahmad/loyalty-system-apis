...
import { OFFERS_ACCESS_KEY } from './offers-access.decorator';
...
@Injectable()
export class OfferAccessGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // -----------------------
    // STEP 1: Global Offer Permissions
    // -----------------------
    const hasAnyBasicModule = privileges.some(
      (p) => p.module === 'basic_modules',
    );

    const hasOffersModule = privileges.some(
      (p) => p.module === 'basic_modules' && p.name === 'offers_module',
    );

    const canViewOffers =
      hasOffersModule &&
      privileges.some(
        (p) => p.module === 'offers_module' && p.name === 'view_offers',
      );
    const canEditOffers =
      hasOffersModule &&
      privileges.some(
        (p) => p.module === 'offers_module' && p.name === 'edit_offers',
      );
    const canCreateOffers =
      hasOffersModule &&
      privileges.some(
        (p) => p.module === 'offers_module' && p.name === 'create_offers',
      );
    const canDeleteOffers =
      hasOffersModule &&
      privileges.some(
        (p) => p.module === 'offers_module' && p.name === 'delete_offers',
      );
    ...
    const determineAccess = (canAction: boolean) => {
      ...
      if (!hasOffersModule) return false;
      if (!canAction && hasTenantOrBUAccess) return false;
      return true;
    };

    const checkAccessForView = determineAccess(canViewOffers);
    const checkAccessForEdit = determineAccess(canEditOffers);
    const checkAccessForDelete = determineAccess(canDeleteOffers);
    const checkAccessForCreate = determineAccess(canCreateOffers);

    // -----------------------
    // STEP 4: Attach Permissions to Request
    // -----------------------
    // User can manage Offers if they have either:
    // 1) Global Offer permissions, or
    ...
    request.permission = {
      ...
      allowAllBU,
      tenantName,
      canViewOffers: canViewOffers ? true : checkAccessForView,
      canEditOffers: canEditOffers ? true : checkAccessForEdit,
      canCreateOffers: canCreateOffers ? true : checkAccessForCreate,
      canDeleteOffers: canDeleteOffers ? true : checkAccessForDelete,
    };
